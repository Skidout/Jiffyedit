...
bool hasres = false, fheight = false /* found height indicator */, fwidth = false, logging = false, fclsp = false, shotcutb = false, pitivib = false, overwrite = false, losslessb = false, vidcutb = false, openshotb = false;
...
inline void melt(vector <string> argv) {
	...
	if (logging) {log("Rounding timestamps...");}
	...
	if (logging) {
		for (i1 = 0; i1 < clparr.size(); i1++) {
			log("clips: ");
			log("	" + to_string(clparr.at(i1)));
		}
	}


	cout << "Generating MLT XML file..." << endl;
	if (logging) {log("Generating MLT XML file...");}
	...
	vector <string> plsflt;// playlist filters
	...
	for (i1 = 0; i1 < plsflt.size(); i1++) {
		out.push_back(plsflt.at(i1));
	}
	...
	outpath = remquo(fs::temp_directory_path().generic_string()) + "/jiffyedit/mlt-0.mlt";
	for (i1 = 0; fs::exists(outpath); i1++) {
		outpath = replace(outpath, to_string(i1), to_string(i1 + 1));
	}

	fs::create_directory(fs::temp_directory_path()/"jiffyedit");
	...
	s1 = "melt -progress \"PATH\"";
	s1 = replace(s1, "PATH", outpath);
	if (logging) {log("MLT cmd: " + s1);}
	system(s1.c_str());
...
inline void vidcut() {
	...
	if (logging) {log("Generating VCP file...");}
...
inline void lossless() {
	vector <string> out;

	cout << "Writing LLC file..." << endl;
	if (logging) {log("Writing LLC file...");}
...
inline void pitivi() {
	...
	if (logging) {log("Generating XGES file...");}

	b1 = true;
	while (b1) { // replace all spaces with %20 code, other codes are probably necessary but until i know that i will assume otherwise
		pos = -1;
		pos = path2.find(" ");
		if (pos == -1) {
			b1 = false;
	...
	for (i1 = 0; i1 < clparr.size(); i1 = i1 + 2) {
		...
		for (i2 = 0; i2 < fltarr.size(); i2++) {
			...
			audfltb = false;
			vidfltb = false;
			if (logging) {log("fltcmd for " + fltarr.at(i2).call + ": " + fltcmd);}
...
int main(int argc, const char * arga[]) {
	...
	for (i1 = 0; i1 < argv.size(); i1++) {
		if (argv.at(i1).find("-i") == 0) { // get the full path of the input
			i1++;
			if (argv.at(i1).find("/") == 0) {
				path = argv.at(i1);
				break;
			}
			else if (argv.at(i1).find(":/") == 1) {
				path = argv.at(i1);
				break;
			} // windows full paths
			else if (argv.at(i1).find("./") == 0) {
				argv.at(i1).erase(0, 1);
				argv.at(i1).insert(0, fs::current_path().generic_string());
				path = argv.at(i1);
				break;
			} else if (argv.at(i1).find("~/") == 0) {
				argv.at(i1).erase(0, 1);
	...
	for (i4 = 0; i4 < ls.size(); i4++) {
		...
		if (temptype.find("ignore") >= 0) {
			...
			} else if (temptype.find("playlistfilterer") == 0) {
				plugin tmpflt;
				tmpflt.filter = true;
				tmpflt.plsflt = true;
				getline(getplgs, tmpflt.name);
	...
	for (i1 = 0; i1 < argv.size(); i1++) {
		if (argv.at(i1).find("--help") == 0 or argv.at(i1).at(0) == '?' or argv.at(i1).find("-help") == 0 or (argv.at(i1).find("-h") == 0 and argv.at(i1).size() < 3)) {help(0);}
	}
	...
	for (i1 = 1; i1 < argv.size(); i1++) {
		...
		if (argv.at(i1).find("-i") == 0) {
			...
			if (c1 == ']') {
				...
				for (i2 = 0; i2 < plugls.size(); i2++) {
					if (plugls.at(i2).call.find(argv.at(i1)) == 0 and not plugls.at(i2).filter) {
						...
						fclipr = true;
						break;
					} else if (plugls.at(i2).call.find(argv.at(i1)) == 0 and plugls.at(i2).filter and not plugls.at(i2).plsflt) {
						fltarr.push_back(plugls.at(i2));
					} else if (plugls.at(i2).plsflt) {
						plsfltarr.push_back(plugls.at(i2));
					}
				}
			} else {
				s1 = argv.at(i1);
			...
			else if (argv.at(i1).find("-log") == 0) {}
			else if (argv.at(i1).find("-no-reset") == 0) {progress = "\n";}
			else if (argv.at(i1).find("-o") == 0) {
				i1++;
				if (argv.at(i1).find("/") == 0) {}
				else if (argv.at(i1).find(":/") == 1) {} // windows full paths
				else if (argv.at(i1).find("./") == 0) {
					argv.at(i1).erase(0, 1);
					argv.at(i1).insert(0, fs::current_path().generic_string());
				} else if (argv.at(i1).find("~/") == 0) {
		...
		else if (argv.at(i1).find("mlt") == 0) {meltb = true;}
	}
	if (not fclipr) {
		cout << "Fatal error: clipper not found." << endl;
		exit(4);
	}
	if (not shotcutb and not pitivib and not losslessb and not vidcutb and not meltb) {
	...
	path = remquo(path);
	fs::path vidpath(path);
	if (not fs::exists(vidpath)) {
	...
	else if (vidcutb) {outpath = chfsfx(outpath, "vcp");}
	if (logging) {log("outpath: " + outpath);}

	fs::path xmlpath(outpath);
	if (fs::exists(xmlpath) and not overwrite and not meltb) {
	...
	secdur = stof(totlen);
	if (logging) {log("total length: " + totlen);}
	...
	cmd4.append(" ");
	if (logging) {cmd4.append("-log");}
	cmd4.append(" ");
	cmd4.append(plugls.at(clpint).args);
	if (logging) {log("clipper cmd: " + cmd4);}
	FILE * inclps;
	...
	while (not b1) {
		while (fgets(buf, 1024, inclps) != NULL) {
			...
			if (s1.find("clipstart: ") == 0) {
				...
				if (logging) {log(s1 + " (from clipper: " + plugls.at(clpint).name + " with call: " + plugls.at(clpint).call + ")");}
				exit(6);
		...
		if (b2) {
			...
			if (logging) {log("Fatal error: Final clipstart with no matching clipend from clipper: " + plugls.at(clpint).name);}
			exit(6);
		}
		pclose(inclps);
		inclps = NULL;
	...
	if (clparr.size() == 0) {
		...
		if (logging) {log ("Fatal error: No clips from clipper: " + plugls.at(clpint).name + " with call: " + plugls.at(clpint).call);}
		exit(6);
	}
	cout << endl;
	...
	if (logging) {log("chanls: " + to_string(chanls));}
	...
	if (logging) {log(replace("vid base: " + s1, "/", " "));}
	...
	if (logging) {log(replace("aud base: " + s1, "/", " "));}
	...
	pclose(invbr);
	if (logging) {log("vid bitrate: " + to_string(vbr));}
...
inline void shotcut() { // Shotcut/MLT XML format
	f2 = fpsnum / fpsden; // find how often a new frame occurs. this more convoluted method is necessary for decimal framerates.
	onefrm = 1.0 / f2;
