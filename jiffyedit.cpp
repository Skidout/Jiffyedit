...
void shotcut() {
	...
	*/
	...
	/*
	if (silarr[count5] != secdur) { // if the last silence end is less than the totaldur, make the totaldur the end of the final clip. if not, the silence end is junk and wont be used.
		count2++;
		silarr[count2] = secdur; count5++; count5++;
	}*/
...
int lines = 0;
float tempf, tempf2, tempf3;
float valfram[1000]; // list of valid frames float values
float silarr[1000000];
int silcnt = 0;
int decbels = 20;
float fpsf;
...
float tonrfram() { // to nearest valid frame. shotcut needs timestamps to be frames but in a decimal format. weird
	tempf = tofrmvar;
	for (tempf; tempf >= 1.0; tempf = tempf - 1.0) {
		if (tempf <= 1.0) {break;}
	}
	
	tofrmvar = tofrmvar - tempf; // tempf is the stuff after the .
	tempf2 = 2.0;
	temp = 0;
	for (count2 = 0; count2 < 30; count2++) {
		if (tempf - valfram[count2] < 0.0) {
			tempf3 = tempf - valfram[count2];
			tempf3 = tempf3 * -1.0; // if the difference is negative, make it positive
			if (tempf3 < tempf2) {
				tempf2 = tempf3;
				temp = count2;
			}
		}
		else if (tempf - valfram[count2] < tempf2) {
			tempf2 = tempf - valfram[count2];
			temp = count2;
		}
	}
	tofrmvar = tofrmvar + valfram[temp];
	...
	if (inmeta != NULL) {
		...
		while (fgets(buf, 1024, inmeta) != NULL) { // this loop sets buf = the current line of the datastream. fgets is for plaintext data only. the 1024 specifies to read a max of 1024 bytes, so as not to overflow (at least i think so anyway). lastly, the datastream to read from is specified. the != null makes it so that when the datastream reaches its end, the loop ends, so as to prevent overflow
			...
			if (boool < 0) {
				...
				for (count3 = 0; count3 < len; count3++) { /// search for fps by looking for a number followed by a ' ' followed by 'f' then 'p' then 's'
					if (hasfps) {break;}
					temp = count3;
					if (isnum()) {
						temp = count3 + 1;
						...
						if (tempc == ' ') {
							...
							if (tempc == 'f') {
								...
								if (tempc == 'p') {
									...
									if (tempc == 'p') {
										hasfps = true;
										temp = count3; /// count3 should be the last number of the fps
										while (isnum() and temp > 0 and ffps == false) {
											temps = metaline[temp];
											fps.append(temps);
											temp --;
										}
										ffps = true;
										reverse(fps.begin(), fps.end());
									}
								}
							}
						}
...
void shotcut() {
	...
	if (ffps == false) {
		cout << "FPS not found. Fatal.";
		exit(3);
	}
	
	stringstream sttof2(fps);
	if (sttof2 >> fpsf) {} else {
		cout << endl << "Error: Failed to convert fps string to fps int. This is fatal." << endl;
		exit(2);
	}
	valfram[0] = 0.000;
	temp = 1;
	for (float count4 = 1.0; count4 < fpsf; count4 = count4 + 1.0) {
		valfram[temp] = count4 / fpsf;
		if (temp > 999) {
			cout << endl << "FATAL ERROR: Too many FPS," << endl;
			exit(1);
		}
		temp++;
	}
...
*/
	...
	string fin[30000]; // final list to write because shotcut wants total time before it can be calculated
	string out[30000]; // holds the chains
	string playlist[30000]; // the playlist section goes directly after the chains and lists where each of the chains begins and ends
	
	
	fin[finlines] = "<?xml version=\"1.0\" standalone=\"no\"?>"; finlines++; // All strings put directly into the output do not seem to change regardless of the video in question.
	fin[finlines] = "<mlt LC_NUMERIC=\"C\" version=\"7.8.0\" title=\"Shotcut version 22.09.23\" producer=\"main_bin\">"; finlines++;
	
	var1 = "  <profile description=\"automatic\" width=\"WIDTH\" height=\"HEIGHT\" progressive=\"1\" sample_aspect_num=\"1\" sample_aspect_den=\"1\" display_aspect_num=\"ASPWID\" display_aspect_den=\"ASPHEI\" frame_rate_num=\"FPS\" frame_rate_den=\"1\" colorspace=\"CLSPC\"/>";
	
	sprstr = var1; substr = "WIDTH"; repstr = width; var1 = replace();
	sprstr = var1; substr = "HEIGHT"; repstr = height; var1 = replace();
	sprstr = var1; substr = "ASPWID"; repstr = aspwid; var1 = replace();
	sprstr = var1; substr = "ASPHEI"; repstr = asphei; var1 = replace();
	sprstr = var1; substr = "FPS"; repstr = fps; var1 = replace();
	sprstr = var1; substr = "CLSPC"; repstr = clspc; var1 = replace();
	...
	totimvar = secdur; dur = toanatim();
	totimvar = secdur - valfram[1]; durm1 = toanatim();
	...
	for (int count = 0; count < silcnt and lines < 29000; count = count + 2) {
		...
		temps = "    <property name=\"shotcut:caption\">audiotest.mp4</property>";
